...
vendored `okf` skill (spec at `${CLAUDE_PLUGIN_ROOT}/skills/okf/reference/SPEC.md`).
...
7. Validate when the change is structural (new dirs, renames): `uv run "${CLAUDE_PLUGIN_ROOT}/skills/validate/scripts/okf_validate.py" brain`.
...
- **Target concept doesn't exist during an update**: create it (okf templates at `${CLAUDE_PLUGIN_ROOT}/skills/okf/templates/`), then apply the update
...
- `${CLAUDE_SKILL_DIR}/scripts/list_stale_and_unverified.py` - derives §5.5 staleness and §5.3 trust tiers from the bundle; shared with brain-sweep and brain-validity so one policy applies everywhere
- `${CLAUDE_PLUGIN_ROOT}/skills/okf/SKILL.md` + `reference/SPEC.md` - format authority (produce/maintain/consume)
- `${CLAUDE_PLUGIN_ROOT}/skills/okf/templates/` - concept/index/log templates
- `${CLAUDE_PLUGIN_ROOT}/skills/validate/scripts/okf_validate.py` - conformance checker
