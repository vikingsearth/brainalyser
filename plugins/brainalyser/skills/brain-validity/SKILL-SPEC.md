...
- **Purpose**: utility
- **Complexity**: simple
- **Description**: Brain validity audit - okf conformance, reality-drift checks against GitHub, the portfolio, and the logs, plus a clustering gap-finder for missing links. Proposes only, never mutates; findings land as a checkbox report in `inbox/`. Runs from the weekly-brain-validity scheduled task or on demand.
